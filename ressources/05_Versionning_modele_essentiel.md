...
|---|---|---|
| `model_name` | `.json` métadonnées (fallback : nom du fichier) | Nom lisible du modèle servi |
...
> ⚠️ Si ton `.json` de M1-B1 n'a pas de clé `model_name` (elle n'était pas
> dans le contrat minimum), deux options : l'ajouter au `.json` (recommandé),
> ou la dériver du nom de fichier avec `metadata.get("model_name", ...)` —
> ton `/info` ne doit **jamais** planter pour une clé optionnelle absente.
...
> **test `test_info_exposes_metadata`** doit vérifier que les clés du
> tableau ci-dessus sont **présentes et non-nulles** — sinon le déploiement
> est refusé. C'est exactement ce qu'on automatisera comme gate CI/CD en M5.
