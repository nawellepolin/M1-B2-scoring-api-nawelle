...
> Brief associé : **M1-B2**.
> Mode : individuel, sync mercredi après-midi (2 h 15) + async jeudi/vendredi (6 h).
...
[`Formation-SIMPLON-IA/ia-dev-id-parcours-m1-b2`](https://github.com/Formation-SIMPLON-IA/ia-dev-id-parcours-m1-b2).
...
1. Va sur [`Formation-SIMPLON-IA/ia-dev-id-parcours-m1-b2`](https://github.com/Formation-SIMPLON-IA/ia-dev-id-parcours-m1-b2)
...
> ⚠️ **D'abord, récupère ton modèle de M1-B1** (`pyrenex_risk_v2.joblib` +
> `pyrenex_risk_v2.json`) et place-le dans `models/`. Le squelette M1-B2 ne
> le contient pas — c'est **ton** artefact de M1-B1. **Sans lui, uvicorn
> refuse de démarrer** (comportement voulu) et pytest skippe les tests.

```bash
git clone git@github.com:<ton-user>/M1-B2-scoring-api-<prenom>.git
cd M1-B2-scoring-api-<prenom>
python -m venv .venv && source .venv/bin/activate
# variante uv : uv venv --python 3.11 && source .venv/bin/activate
pip install -r requirements.txt
# variante uv : uv pip install -r requirements.txt   (un `uv venv` n'embarque pas pip)
cp ../M1-B1-scoring-<prenom>/models/pyrenex_risk_v2.* ./models/
uvicorn app.main:app --reload          # → démarre sans erreur, /health répond 200
pytest                                  # → tests "skipped" tant que les TODO ne sont pas faits : normal
```

Si `uvicorn` démarre et que `/health` répond 200, ton poste est prêt.
